  ...
  return (
    <div className="container mx-auto px-4 my-6 relative">
      ...
      <div className="flex items-center gap-4 md:gap-6 overflow-scroll scrollbar-none transition-all" ref={scrollElement}>
        <button
          className="bg-white shadow-md rounded-full p-1 absolute left-0 text-lg hidden md:block transition-all transform hover:scale-105"
          onClick={scrollLeft}
        >
          <FaAngleLeft />
        </button>
        <button
          className="bg-white shadow-md rounded-full p-1 absolute right-0 text-lg hidden md:block transition-all transform hover:scale-105"
          onClick={scrollRight}
        >
          <FaAngleRight />
        </button>
        ...
        {loading ? (
          ...
          data.map((product, index) => (
            <Link to={`product/${product?._id}`} key={product?._id} className="w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex hover:scale-105 transition-all duration-300">
